...
- **Writing to Notion is done from an agent session, not by hand.** The runtime code only reads; there is no write path in the repo and there should not be. To change content, run a throwaway script from the project root (so `@notionhq/client` resolves) with `bun --env-file=.env`. Recipe for a `files` property: `fileUploads.create({ mode: "single_part", filename, content_type })` → `fileUploads.send({ file_upload_id, file: { data: new Blob([bytes]), filename } })` → `pages.update({ page_id, properties: { <Prop>: { files: [{ type: "file_upload", file_upload: { id }, name }] } } })`. Find a row by querying `databases.retrieve(...).data_sources[0].id` then `dataSources.query`. Delete the script afterwards. A content edit will not appear locally until the cache entry expires or the dev server restarts.
...
- The **About copy is the `About` property** on the Profile row, one paragraph per line, with the inline markdown subset allowed. It used to be the Profile page body, which the owner could not find: a row with no About field and the prose hidden a click deeper. Page bodies are still the source for experience bullets, project case studies and post bodies, where the block pipeline earns its keep; a few paragraphs of prose does not need one.
...
- Skills render in full, server-side, and every chip is plain. Three treatments were tried and rejected: a show-few/show-all toggle (hides content behind a click), marking only primary tools (the mixed weights read as noise), and hardcoding `chip--strong` on all of them (made the `Highlight` property dead code). The markup keeps the `skill.highlight ? "chip chip--strong" : "chip"` conditional and **`Highlight` is unticked on all 75 Skills rows in Notion**, so emphasis is a data decision, not a code change. Note the consequence: with nothing ticked, JSON-LD `knowsAbout` is empty and the assistant drops its "primary tools" line.
...
- Scroll-driven CSS (`animation-timeline`) powers the hero rise and the experience rail, behind `@supports` so unsupporting browsers get the finished state. Both are switched off outright under `prefers-reduced-motion`, since a scroll timeline ignores `animation-duration`.
- A past role's dot (`.timeline-dot--past`) turns cyan the moment the fill reaches it: same `view(58% 42%)` inset, so its 9px cover range crosses the line the segment above it finishes on. It fills to match the current role's dot, so the pulse alone marks "now". Its border and background live in CSS, not Tailwind utilities, because utilities win over the components layer and would pin it grey.
- **The rail is dot to dot.** Each `li` owns `::before` (track, `--rail`, 2px) and `::after` (cyan fill) from its own dot centre (`top: 11.5px`) to the next item's (`bottom: -11.5px`); `:last-child` hides both, so the line stops on the last marker and never runs on through that role's bullets. Extending it to `bottom: 0` was tried and looked like a bar hanging off the bottom of the timeline: with two roles that is most of a viewport of line beside nothing.
- The fill is one coherent boundary despite being per segment. `animation-timeline: view(58% 42%)` insets the scrollport to a zero-height line 58% down the screen, so `cover 0% cover 100%` is exactly that segment's own height and it fills while the line crosses it. Segments above the line are full, below it empty, so the boundary is pinned to the line and descends with the scroll. **The inset must be the argument of `view()`.** The `view-timeline-inset` property applies only to named timelines (`view-timeline-name`); set beside an anonymous `view()` it is silently ignored and the fill degrades to plain cover over viewport plus segment, which crawls and only reaches a dot as it leaves the top of the screen. That silent no-op was the real bug under every earlier "fix", and computed styles will happily show the property as set while it does nothing.
- What went wrong before, in order, so nobody repeats it: `cover 0% cover 45%` completed every segment before it was scrolled to (uniform cyan); the track used `--line` at 8%, invisible at 1px, so a partial fill looked severed; the "fix" for severed extended the last segment to `bottom: 0` (a bar hanging off the timeline); and the inset was set as a property the anonymous timeline never reads. **Verify with the formula, not the eye alone:** at a given scroll, actual `scaleY` must equal `(0.58 × viewportHeight − segmentTop) / segmentHeight` clamped to [0, 1]. If it instead equals `(viewportHeight − segmentTop) / (viewportHeight + segmentHeight)`, the inset is not being applied. Then confirm on a full-section screenshot and on the framing where the last dot sits near the top.
...
- `vercel.json` pins two things Vercel gets wrong on its own: `framework: nextjs` (the project preset was left on Vite from the v1 site, so builds looked for `dist`) and `installCommand: bunx bun@1.4.2 install`. The build image ships an older bun that cannot read the `lockfileVersion: 2` lockfile bun 1.4 writes, so the pin keeps CI and local on the same bun. Bump that version string when you upgrade bun locally.
- A Blob store must be connected to the project for every environment, including Development, which is the one `vercel env pull` reads.
