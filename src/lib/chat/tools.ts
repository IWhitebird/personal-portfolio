...
/**
 * Server tools carry `execute` and close over the content the route fetched.
 * Page tools have no `execute`: the browser runs them via `onToolCall` and
 * reports back with `addToolOutput`.
 *
 * Schemas stay flat (strings, enums, no defaults/transforms) for Groq's JSON-schema validation.
 */
export function buildTools(content: SiteContent) {
  return {
    search_content: tool({
      description:
      ...
      execute: async ({ query }) => {
        const hits = searchContent(content, query);
    ...
    get_experience: tool({
      ...
      execute: async ({ company }) => getExperience(content, empty(company)),
    }),
    ...
    get_projects: tool({
      ...
      execute: async ({ name }) => getProjects(content, empty(name)),
    }),
    ...
    get_skills: tool({
      ...
      execute: async ({ category }) => getSkills(content, empty(category) ?? "all"),
    }),

    get_contact_info: tool({
      description: "Email, location, availability, profile links, and the résumé URL.",
      inputSchema: z.object({}),
      execute: async () => getContactInfo(content),
    }),
